...
class ContainerApiGenerator extends GeneratorCommand implements ComponentsGenerator
{
    ...
    public function getUserInputs(): ?array
    {
        ...
        $this->call('nucleus:generate:resource', [
            '--section' => $sectionName,
            '--container' => $containerName,
            '--file' => $containerName . 'Resource',
            '--model' => $model,
            '--full' => false,
        ]);
        ...
        $this->call('nucleus:generate:factory', [
            '--section' => $sectionName,
            '--container' => $containerName,
        ...
        $doctype = $this->checkParameterOrChoice(
            'doctype',
            'Select the type for all API endpoints',
            ['private', 'public'],
            0
        );
        ...
        $url = Str::lower(
            $this->checkParameterOrAsk(
                'url',
                'Enter the base URI for all API endpoints (foo/bar/{id})',
                Str::kebab($models)
            )
        );
        $url = ltrim($url, '/');
        ...
        $generateEvents = $this->checkParameterOrConfirm(
            'events',
            'Do you want to generate the corresponding CRUD Events for this Container?',
            false
        );
        $generateListeners = false;
        if ($generateEvents) {
            $generateListeners = $this->checkParameterOrConfirm(
                'listeners',
                'Do you want to generate the corresponding Event Listeners for this Events?',
                false
            );
        }

        $generateTests = $this->checkParameterOrConfirm(
            'tests',
            'Do you want to generate the corresponding Tests for this Container?',
            false
        );
        ...
        foreach ($routes as $route) {
            ...
            if ($generateEvents) {
                $this->call('nucleus:generate:event', [
                    '--section' => $sectionName,
                    '--container' => $containerName,
                    '--file' => $route['event'],
                    '--model' => $model,
            ...
            if ($generateTests) {
                $this->call('nucleus:generate:test:unit', [
                    '--section' => $sectionName,
                    '--container' => $containerName,
                    '--file' => $route['unittest'],
                    '--model' => $model,
                ...
                $this->call('nucleus:generate:test:functional', [
                    '--section' => $sectionName,
                    '--container' => $containerName,
