...
trait EnumConcern
{
     ...
     */
    public static function all(string $method = '', bool $keyByValue = false): Collection
    {
        if (!method_exists(self::class, $method)) {
            return collect(self::cases())->pluck('value', 'name');
        }

        return collect(self::cases())->mapWithKeys(function ($item) use ($method, $keyByValue) {
            return [$keyByValue ? $item->value : $item->name => self::from($item->value)->$method()];
        });
    }
     ...
     */
    public static function caseByValue(null|string|int $case, string $method = ''): string|bool
    {
    ...
    public static function valuesArray(string $method = ''): array
    {
        return self::all($method)->flatten()->toArray();
    }
    ...
    /**
     * Returns an array of options based on the getLabel() method of the current class.
     *
     * @param array $excludes
     * @return array
     */
    public static function toOption(array $excludes = []): array
    {
